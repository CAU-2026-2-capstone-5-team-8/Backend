...
INSERT INTO backend.question(topic_id, measurement_area, difficulty, prompt, concept_id, version, active, demo_key)
SELECT t.id, 'VOCABULARY', 2,
       '"교착 상태(deadlock)"라는 용어의 뜻을 알고 있습니까?',
       'deadlock', 'demo-question-v2', true, 'os-vocab-1'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;

INSERT INTO backend.question(topic_id, measurement_area, difficulty, prompt, concept_id, version, active, demo_key)
SELECT t.id, 'VOCABULARY', 3,
       '"세마포어(semaphore)"라는 용어의 뜻을 알고 있습니까?',
       'semaphore', 'demo-question-v2', true, 'os-vocab-2'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
...
SELECT t.id, 'VOCABULARY', 1,
       ...
       'virtual memory', 'demo-question-v2', true, 'os-vocab-3'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
...
SELECT t.id, 'BACKGROUND_KNOWLEDGE', 3,
       ...
       'page fault', 'demo-question-v2', true, 'os-knowledge-1'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
...
SELECT t.id, 'BACKGROUND_KNOWLEDGE', 4,
       ...
       'round robin scheduling', 'demo-question-v2', true, 'os-knowledge-2'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
...
SELECT t.id, 'BACKGROUND_KNOWLEDGE', 2,
       ...
       'mutex', 'demo-question-v2', true, 'os-knowledge-3'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
...
SELECT t.id, 'COMPREHENSION', 3,
       ...
       'scheduling', 'demo-question-v2', true, 'os-comprehension-1'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
...
SELECT t.id, 'COMPREHENSION', 2,
       ...
       'cache hit ratio', 'demo-question-v2', true, 'os-comprehension-2'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
...
SELECT t.id, 'COMPREHENSION', 4,
       ...
       'starvation', 'demo-question-v2', true, 'os-comprehension-3'
FROM backend.topic t WHERE t.code='OS' ON CONFLICT(demo_key) DO UPDATE
SET concept_id = EXCLUDED.concept_id
WHERE question.concept_id IS NULL;
