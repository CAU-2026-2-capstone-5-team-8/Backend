...
이 API까지가 지금 실제로 되는 것이고, **답변을 저장하거나 채점하는 기능은 아직 없다.**
...
| `assessment_answer` | 테이블만 있고 비어있음 | 저장하는 API가 아직 없음 |
...
- ❌ 진단 문제에 답 제출하기 (`PUT .../answers/...` 없음)
- ❌ 세션 다시 조회하기 (`GET /api/assessments/{sessionId}` 없음)
